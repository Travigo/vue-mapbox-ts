 ...
 */
export function bindEvents(vueElement: any, mapboxElement: any, events: any[] = [], layerId: any = null) {
  const { $listeners: vueEvents } = vueElement;
  // eslint-disable-next-line no-param-reassign
  vueElement.$$events = Object.keys(vueEvents).reduce(($$events: any[], vueEvent: any) => {
    const originalEvent = vueEvent.replace(/^mb-/, '');
    if (!events.includes(originalEvent)) {
      return $$events;
    }

    const handler = (...payload: Array<any>) => {
 ...
 * @return {void}
 */
export function unbindEvents(vueElement: any, mapboxElement: any, layerId: any = null) {
  vueElement.$$events.forEach(([event, handler]: [any, any]) => {
    // If layerId is not null, all events must be
    // delegated from the map to the given layerId
    if (layerId) {
